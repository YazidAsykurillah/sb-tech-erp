...
class DatatablesController extends Controller
{
    ...
    public function getPOVendors(Request $request)
    {
        ...
        $data_po_vendors = Datatables::of($po_vendors)
            ...
            ->addColumn('balance', function($po_vendors){
                $amount = $po_vendors->purchase_request ? number_format($po_vendors->purchase_request->amount) : 0;
                $paid_invoice_vendor = $po_vendors->paid_invoice_vendor() ? number_format($po_vendors->paid_invoice_vendor()) : 0;
    ...
    public function getInvoiceVendors(Request $request)
    {
        \DB::statement(\DB::raw('set @rownum=0'));
    ...
    public function getInternalRequests(Request $request)
    {
        \DB::statement(\DB::raw('set @rownum=0'));
        $user_role = \Auth::user()->roles()->first()->code; 
        if( $user_role == 'SUP' || $user_role == 'ADM' || $user_role == 'FIN'){
    ...
    public function getQuotationCustomers(Request $request)
    {
        \DB::statement(\DB::raw('set @rownum=0'));
        $user_role = \Auth::user()->roles->first()->code;
        if($user_role == 'SUP' || $user_role == 'ADM' || $user_role=='FIN'){
    ...
    public function getQuotationVendors(Request $request)
    {
        \DB::statement(\DB::raw('set @rownum=0'));
    ...
    public function getSettlements(Request $request)
    {
        ...
        $user_role = \Auth::user()->roles->first()->code;
        if($user_role == 'SUP' || $user_role == 'ADM' || $user_role == 'FIN'){
            $settlements = Settlement::with('internal_request', 'internal_request.requester' ,'category', 'sub_category', 'last_updater')->select([
                \DB::raw('@rownum  := @rownum  + 1 AS rownum'),
                'settlements.*',
            ]);
        }
